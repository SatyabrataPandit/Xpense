...
interface MainChartProps {
    filters: {
        type: 'all' | 'monthly' | 'yearly' | 'daily'; // Added daily
        year: string;
        month: string;
...
export function MainChart({ filters }: MainChartProps) {
    ...
    const { start, end, statusText } = useMemo(() => {
        ...
        if (filters.type === 'daily') {
            s = new Date(filters.date);
            s.setHours(0, 0, 0, 0);
            e = new Date(filters.date);
            e.setHours(23, 59, 59, 999);
            text = `Daily Breakdown: ${s.toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}`;
    ...
    useEffect(() => {
        ...
        const unsub = onSnapshot(q, (snapshot) => {
            ...
            const getLabels = (d: Date) => {
                const label = filters.type === 'daily' ? d.toLocaleDateString('en-IN', { hour: '2-digit' }) : 
                            filters.type === 'monthly' ? d.toLocaleDateString('en-IN', { day: '2-digit' }) : 
            ...
            // Pre-fill categories to ensure the chart shows the full range
            if (filters.type !== 'daily') {
            ...
            snapshot.docs.forEach((doc) => {
                ...
                const { label, full } = getLabels(tx.date.toDate());
                
                if (!chartMap[label]) {
                    chartMap[label] = { dateLabel: label, fullDate: full, income: 0, expense: 0 };
                }
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm w-full flex flex-col items-center overflow-hidden">
            ...
            <div className="w-full aspect-video md:aspect-21/9 min-h-75 max-h-100 flex justify-center items-center">
                <ResponsiveContainer width="99%" height="100%">
                    {/* Switch to BarChart for Daily view as it looks better for single-day entries */}
                    {filters.type === 'daily' ? (
...
function CustomTooltip({ active, payload }: any) {
    if (active && payload && payload.length >= 1) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const received = Number(payload.find((p: any) => p.dataKey === 'income')?.value || 0);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const spent = Number(payload.find((p: any) => p.dataKey === 'expense')?.value || 0);
