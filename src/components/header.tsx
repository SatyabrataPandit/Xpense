...
import { LayoutDashboard, Plus, ChevronDown, Check, Calendar as CalendarIcon } from "lucide-react";
import Link from 'next/link';
import { motion, AnimatePresence } from 'framer-motion';

interface HeaderProps {
    filters: {
        type: 'all' | 'monthly' | 'yearly' | 'daily';
        year: string;
        month: string;
        date: string;
    };
        ...
        <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 w-full">
            ...
            <div className="flex-1 order-1">
                ...
                <p className="text-[11px] md:text-xs font-bold text-slate-400 dark:text-slate-500 flex items-center gap-2 tracking-wide uppercase">
                    Monitor your balance and precise daily cash flow.
                </p>
            </div>
                    ...
                    {['daily', 'monthly', 'yearly', 'all'].map((type) => (
                        <button
                            ...
                            onClick={() => setFilters(prev => ({
                                ...prev,
                                type: type as 'all' | 'monthly' | 'yearly' | 'daily'
                            }))}
                            className={`flex-1 sm:flex-none px-3 md:px-4 py-2 text-[9px] font-black uppercase tracking-wider rounded-lg transition-all ${filters.type === type
                        ...
                        >
                            {filters.type === 'daily' ? (
                                <div className="flex items-center gap-2 px-3 py-1.5">
                                    ...
                                    <div className="relative flex items-center bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded-lg border border-transparent focus-within:border-indigo-500/20 transition-all">
                                        <input 
                                            type="date" 
                                            value={filters.date}
                                            onChange={(e) => setFilters(p => ({ ...p, date: e.target.value }))}
                                            className="bg-transparent text-[10px] font-black uppercase tracking-tight text-slate-700 dark:text-slate-200 outline-none cursor-pointer"
                                        />
                                        <CalendarIcon size={12} className="text-indigo-500 ml-2 pointer-events-none" />
                                    </div>
                                </div>
                            ) : (
                                <>
